...
import MovieDetails from './components/MovieDeatails.vue';
...
//sama dengan mounted alurnya dari elemen htmlnya di klik
onMounted(() => {
...
<template>
  <div class="fixed w-full h-screen bg-black">
    ...
    <!-- jika tidak ada video akan menampilkan ini  -->
    <div v-if="!showFullVideo">
      ...
      <!-- untuk bagian corusel kategory itu judul nya -->
    ...
    <!-- menampilkan jika video di putar -->
    <div v-if="showFullVideo">
      <!-- ketika di klik akan  menghilangkan tampilan video -->
      <div @click="showFullVideo = false"
        class="absolute z-50 p-2 m-4 bg-white bg-opacity-50 rounded-full cursor-pointer">
        <!-- tombol kembali ketika video di putar -->
        <ChevronLeft fillColor="#FFFFFF" :size="40" />
      </div>
      <video :src="'/videos/' + movie.name + '.mp4'" loop autoplay controls
